...
```{r change it to your path there}
...
```{r Rich's HRS-HCAP data}
data <- read_dta((file.path(data_folder, "HCAPHRS.dta")))
```

```{r keep the HCAP participants only}
df<-data[!is.na(data$HCAP16WGTR),]
```

```{r variables that can be used as the variable to condition on }
# race
...
```{r HRS dementia classification from multiple approaches}
...
```{r link HCAP and the above datasets together}
...
```{r variables we want to stratify on Part 1}
#ageGroup
...
```{r merge the dataset from Cliff, RAND dataset}
...
```{r include Wu dementia classification }
...
```{r merge the cognitive assessments from HCAP data}
...
```{r merge the address, dates and repetition items from HCAP data}
...
```{r variables we want to stratify on Part 2}
#workforce
...
```{r keep the variables for crosswalk and save the dataset}
...
df22$wordlist<-df22$s_imrc2016 # range 0-10, same as the immediate word recall
df22$wordlist2<-df22$imrc_imp2016 # range 0-10, same as the immediate word recall

df22$subtraction<-df22$s_ser72016 # range 0-5
df22$subtraction2<-df22$ser7_imp2016
...
df22$cog_30 <- rowSums(df22[, c("R1ORIENTTIME", "address2", "backCounting", "wordlist",
                                ...
                                "Repetition")], na.rm = TRUE)

df22$cog_30_2 <- rowSums(df22[, c("R1ORIENTTIME", "address2", "backCounting", "wordlist2",
                                "subtraction2", "responsive_naming", "president",
                                "Repetition")], na.rm = FALSE)
# cog_40 
df22$cog_40<- rowSums(df22[, c("R1ORIENTTIME", "address2", "backCounting", "wordlist",
                                "subtraction", "responsive_naming", "president",
                                "delayedRecall", "Repetition")], na.rm = TRUE)
df22$cog_40_2<- rowSums(df22[, c("R1ORIENTTIME", "address2", "backCounting", "wordlist2",
                                "subtraction2", "responsive_naming", "president",
                                "delayedRecall", "Repetition")], na.rm = FALSE)

```

```{r check missingness in TICS items}
sum(is.na(df22$r1mmse_score))
sum(is.na(df22$cog_20))
sum(is.na(df22$cog_27))
sum(is.na(df22$cog_30_2))
sum(is.na(df22$cog_35))
sum(is.na(df22$cog_40_2))

vars <- c("R1ORIENTTIME", "address2", "backCounting", "wordlist2",
          "subtraction2", "responsive_naming", "president",
          "delayedRecall", "Repetition")

na_counts <- sapply(vars, function(v) sum(is.na(df22[[v]])))
na_counts
```

```{r Save the final dataset for analysis}
write_dta(df22, (file.path(data_folder, "/Derived/HRS-HCAPcrosswalkdata.dta")))
